import {Directive, forwardRef, HostListener, Input, Renderer2} from '@angular/core';
...
@Directive({
  ...
  providers: [{
    provide: NG_VALUE_ACCESSOR, useExisting:
      forwardRef(() => VgNumberOnlyDirective),
    multi: true
  }]
})
export class VgNumberOnlyDirective implements ControlValueAccessor {
  ...
  onInput(el: HTMLInputElement): void {
    ...
    const originalNonDigitCount = el.value.match(this.excludedCharsRegex)?.length || 0;
    const noDigitsString = el.value.toString().replace(this.excludedCharsRegex, '');
    const newValue = noDigitsString.toString().replace(this.excludedNonLeadPlusMinusRegex, '');
    const modifiedNonDigitCount: number = newValue.substring(0, start).match(this.excludedCharsRegex)?.length || 0;
    el.value = newValue;
