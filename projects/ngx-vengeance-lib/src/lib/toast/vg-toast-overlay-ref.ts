...
export class VgToastOverlayRef {
  ...
  pushToastBeforeToLowerPosition(): void {
    if (this.toastOverlayRefBefore) {
      ...
      const toastBeforePositionStrategy =
        this.toastOverlayRefBefore.overlayRef.getConfig()
          .positionStrategy as GlobalPositionStrategy;
      const currentPosition = this.toastOverlayRefBefore.getPosition();
      toastBeforePositionStrategy.top(
        currentPosition.top + currentPosition.height + 20 + 'px'
      );
      this.toastOverlayRefBefore.overlayRef.updatePositionStrategy(
        toastBeforePositionStrategy
      );
